...
class MaterialSeekbarPreference : Preference {
    ...
    constructor(
        ...
        context.withStyledAttributes(
            ...
            mMin = getFloat(R.styleable.MaterialSeekbarPreference_minValue, 0f)
            setMax(getFloat(R.styleable.MaterialSeekbarPreference_maxValue, 100f))
            setSeekBarIncrement(
                getFloat(
                    R.styleable.MaterialSeekbarPreference_seekBarIncrement,
                    0f
                )
            )
            mShowSeekBarValue =
                getBoolean(R.styleable.MaterialSeekbarPreference_showSeekBarValue, false)
            mUpdatesContinuously = getBoolean(
                R.styleable.MaterialSeekbarPreference_updatesContinuously,
                false
            )

            mLabelMinWidth =
                getDimensionPixelSize(R.styleable.MaterialSeekbarPreference_labelMinWidth, 0)
            mPrecision = getInt(R.styleable.MaterialSeekbarPreference_precision, 2)
        }
    }
        ...
        this.setOnPreferenceClickListener {
            context.showInputAlert(
                ...
                try {
                    ...
                    Log.e("Slider", "Failed to parse number input", ex)
                    context.toast(
    ...
    fun  /* synthetic access */updateLabelValue(value: Float) {
        if (mSeekBarValueTextView != null) {
            ...
            {
                mSeekBarValueTextView!!.text = "%.${mPrecision}f".format(Locale.ROOT, value)
            }
            else
            {
